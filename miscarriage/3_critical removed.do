...
* Sensitivity meta-analysis removing studies that scored 'citical' in the ROBINS-I risk of bias analysis
...
* Start logging
	...
	log using "$Logdir\3_critical removed", name(critical_removed) replace

*******************************************************************************************

* Change working directory for saving figures

	cd "/file/space/for/figures/"
	...
	* Make the dataframe out of the raw data where possible - for those with just OR(95%CI) provided in the paper put '.' (missing) to fill in later:
		...
		clear
			...
			input float trial str20 author adtype year rt ht rc hc nt nc
				...
				8 "Einarson" 1 2009 122 937 75 937 8 8   			
				9 "Nakhai-Pour" 1 2010 284 1685 4840 54679 9 9 				
				11 "Chan" 1 2011 . . . . 11 11			
				12 "Einarson" 1 2011 8 89 8 89 12 12
				13 "Ban" 1 2012 1982 13151 47258 390665 13 13	
				14 "Klieger-Grossman" 1 2012 32 213 18 212 14 14		
				15 "Kjaesgaard" 1 2013 210 1674 110377 818426 15 15
				16 "Andersen" 1 2014 2883 22884 139201 1256959 16 16		
				17 "Abadie" 1 2015 170 899 668 4447 17 17		
				18 "Johansen" 1 2015 2359 20612 124333 1165124 18 18	
				20 "Almeida" 1 2016 165 1955 1685 28343 20 20
				21 "Evans-Hoeker" 1 2018 16 90 118 1484 21 21	
				22 "Richardson" 1 2019 46 281 140 1405 22 22
				23 "Wu" 1 2019 42 223 617 5228 23 23
				24 "Bahat" 1 2020 . . . . 24 24
				26 "Ankarfeldt" 1 2021 . . . . 26 26
				27 "Kitchin" 1 2022 . . . . 27 27
				28 "Ostenfeld" 1 2022 . . . . 28 28
				29 "Giner-Soriano" 1 2022 . . . . 29 29
				32 "Einarson" 2 2001 16 150 11 150 32 32
				33 "Sivojelezova" 2 2005 14 132 13 132 33 33
				35 "Ban" 2 2012 1539 10132 47258 390665 35 35
				36 "Klieger-Grossman" 2 2012 32 213 18 212 36 36
				37 "Andersen" 2 2013 2883 22884 139201 1256956 37 37 
				38 "Johansen" 2 2015 . . . . 38 38
				39 "Evans-Hoeker" 2 2018 11 57 118 1484 39 39
				40 "Wu" 2 2019 35 179 617 5228 40 40
				41 "Kitchin" 2 2022 . . . . 41 41
				42 "Einarson" 3 2001 18 150 11	150 42 42
				44 "Richardson" 3 2019 46 281 140 1405 44 44
				45 "Bahat" 3 2020 . . . . 45 45
				46 "Ankarfeldt" 3 2021 143 1201 103367 984447 46 46
	...
	* trial (1,2,3...) to "Author (year)" 
		...
		label define trial_lb 	3"Einarson {it:et al.}" ///
								4"Einarson {it:et al.}" ///
	...
	* replace selogor = (((log(upCI in paper))-(log(lCI in paper)))/3.92) if trial === Whichever study you are filling in
		...
		replace logor = log(1.14) if trial ==15
		replace selogor = (((log(1.18))-(log(1.1)))/3.92) if trial ==15 // Kjaesgaard

		replace logor = log(1.27) if trial ==16
		replace selogor = (((log(1.33))-(log(1.22)))/3.92) if trial ==16 // Andersen
	...
	* As we have some gaps where we don't have raw data for meta esize to use, we can use what we calculated in the first stage to fill in the gaps: re-run lines 10-96 and use what you calculated to fill in the gaps in the esize dataset
		...
		replace _meta_es = logor if inlist(trial,8,9,11,12,13,15,16,17,18,20,21,22,23,24,26,27,28,29,35,37,38,39,40,41,44,45,46)
		replace _meta_se = selogor if inlist(trial,8,9,11,12,13,15,16,17,18,20,21,22,23,24,26,27,28,29,35,37,38,39,40,41,44,45,46)
		replace _meta_cil = lci if inlist(trial,8,9,11,12,13,15,16,17,18,20,21,22,23,24,26,27,28,29,35,37,38,39,40,41,44,45,46)
		replace _meta_ciu = uci if inlist(trial,8,9,11,12,13,15,16,17,18,20,21,22,23,24,26,27,28,29,35,37,38,39,40,41,44,45,46)
	...
	* Total number of people in the study
		...
		replace _meta_studysize = 988397 if trial ==26
		replace _meta_studysize = 72280 if trial ==27
		replace _meta_studysize = 4475 if trial ==28
		...
		replace _meta_studysize = 988397 if trial ==46
	...
	* And the meta-analysis forest plot
		...
		forestplot, useopts nostats nowt rcols(_EFFECT _WT) graphregion(color(white)) xlab(0.4,1,9) name("Soph_analysis_RandR_v2_crit_rm", replace)
		
		graph export Soph_analysis_randr_v2_crit_rm.pdf, name(Soph_analysis_RandR_v2_crit_rm) replace

*********************************************************************************************************

* Stop logging

	log close critical_removed
	
	translate "$Logdir\3_critical removed.smcl" "$Logdir\3_critical removed.pdf", replace
	
	erase "$Logdir\3_critical removed.smcl"
